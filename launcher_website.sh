...
# DApps Localhost Launcher — Clean Full Rewrite v3.0.0
# Platform : Termux (Android)
# Purpose  : Full-featured, clean, stable launcher for static/frontend/backend/fullstack
# Author   : Rewritten for you (clean, robust, no grep -P)
# Usage    : sudo? NO. Run as normal Termux user.
# Notes    : Read code-first; explanation below the code.
set -euo pipefail
IFS=$'\n\t'
...
get_free_port() {
  ...
  for i in $(seq 0 400); do
    ...
    if ! echo "$used" | grep -q -E "[: ]$p\b"; then
      echo "$p"; return 0
    fi
  done
  return 1
...
# -----------------------
# Sync: copy source (usually /storage) -> Termux HOME workspace
# -----------------------
sync_to_home() {
  local src="$1" name="$2" dst="$PROJECTS_DIR/$name"
  # if already under PROJECTS_DIR, return as-is
...
build_frontend() {
  ...
  msg info "Tidak ada build script di $dir"
  return 2
}
...
start_service() {
  ...
  echo "$port" > "$portf"
  sleep 1
  if kill -0 "$(cat "$pidf")" 2>/dev/null; then msg ok "$label started (PID $(cat "$pidf"), Port $port)"; return 0; else msg err "$label gagal start. cek: $logf"; rm -f "$pidf" "$portf"; return 1; fi
}
stop_service() {
...
supervise_service() {
  ...
  msg ok "Supervisor started (background) for $proj/$label"
}
...
# -----------------------
# Watch & Auto-sync (inotifywait or polling)
# -----------------------
watch_and_sync() {
  ...
  if require_cmd inotifywait; then
    ...
    msg info "inotifywait tidak tersedia; fallback polling setiap ${interval}s"
    local last
...
add_project_interactive() {
  header; echo "Tambah Project Baru"
  ...
  if [ "$auto_sync" = "1" ] && [ ! -z "$src" ] && [ -d "$src" ]; then
    if require_cmd inotifywait; then
      ...
      msg warn "inotifywait tidak terpasang; auto-watch tidak aktif"
    fi
  fi
}
...
run_project_flow() {
  ...
  case "$P_TYPE" in
    static)
      ...
      local cmd; cmd=$(detect_start_command "$static_dir" "static"); cmd=${cmd:-"python3 -m http.server \$PORT"}
      start_service "$P_NAME" "static" "$static_dir" "$P_FE_PORT" "$cmd"
      ;;
    frontend)
...
status_flow() {
  header; echo "Status semua project"
...
edit_project_flow() {
  header; echo "Edit project"
  if ! name=$(select_project_prompt); then wait_key; return; fi
  load_project "$name"
  echo "Kosongkan input untuk tetap sama"
  ...
  read -rp "Supervise (0/1) [$P_SUPERVISE]: " sp; sp=${sp:-$P_SUPERVISE}
...
update_from_git_flow() {
  header; echo "Update from Git"
  if ! name=$(select_project_prompt); then wait_key; return; fi
  load_project "$name"
  if [ ! -d "$P_PATH/.git" ]; then msg err "Bukan repo git: $P_PATH"; wait_key; return; fi
  (cd "$P_PATH" && git pull --rebase) && msg ok "Git pull selesai" || msg err "Git pull gagal"
  wait_key
}

export_config_flow() {
  header; echo "Export config"
  read -rp "Nama file output [dapps-export.json]: " fname; fname=${fname:-dapps-export.json}
  if require_cmd jq; then
    jq -R -s -c 'split("\n")|map(select(length>0))|map(split("\t")|{name:.[0],path:.[1],type:.[2],fe_dir:.[3],be_dir:.[4],fe_port:.[5],be_port:.[6],fe_cmd:.[7],be_cmd:.[8],auto_sync:.[9],supervise:.[10]})' "$CONFIG_FILE" > "$fname"
  else
    awk -F'\t' 'BEGIN{print "["}{if(NR>1)print ","; printf("{\"name\":\"%s\",\"path\":\"%s\",\"type\":\"%s\"}",$1,$2,$3)}END{print "]"}' "$CONFIG_FILE" > "$fname"
  fi
  msg ok "Config diexport ke $fname"; wait_key
}

import_config_flow() {
  header; echo "Import config"
  ...
  if require_cmd jq; then
    ...
    msg ok "Import selesai"; wait_key
  else
    msg err "jq diperlukan untuk import JSON yang reliable"; wait_key
  fi
}

self_update_flow() {
  header; echo "Self-update launcher"
  if ! require_cmd curl && ! require_cmd wget; then msg err "curl/wget dibutuhkan"; wait_key; return; fi
  ...
  sed -i "1c #!/data/data/com.termux/files/usr/bin/env bash" "$tmp" || true
  chmod +x "$tmp"
  mv -f "$tmp" "$PREFIX/bin/dapps"
  chmod +x "$PREFIX/bin/dapps"
  msg ok "Launcher diperbarui di $PREFIX/bin/dapps"; wait_key
}

uninstall_flow() {
  header; echo "Uninstall launcher"
  if confirm "Yakin uninstall launcher dan bersihkan config/log?"; then
    pkill -f dapps || true
    rm -f "$PREFIX/bin/dapps" "$HOME_DIR/bin/dapps" || true
    rm -f "$CONFIG_FILE"
    rm -rf "$LOG_DIR" "$PROJECTS_DIR"
    hash -r
    msg ok "DApps launcher terhapus bersih"
  else
    msg info "Dibatalkan"
  fi
  wait_key
...
Usage:
  ...
  dapps add --git URL --name NAME [--type frontend] [--supervise] -> add project
  dapps status
  dapps logs <name|#>
  dapps export [file]
  dapps import <file>
  dapps watch <src> <name>   -> start watch & auto-sync (background)
  dapps supervise <name|#>   -> start supervisor for configured roles
  dapps uninstall
  dapps help
EOF
}
...
main_menu() {
  while true; do
    ...
    echo "16) 🛡 Start Supervisors (configured)"
    echo "99) ❌ Uninstall"
    echo "0) Exit"
    read -rp $'\n''Pilih: ' ch
    case "$ch" in
      ...
      14)
         ...
         ;;
      15)
         if ! name=$(select_project_prompt); then wait_key; else load_project "$name"; read -rp "Source to watch (path): " src; src=${src:-$P_PATH}; (watch_and_sync "$src" "$P_NAME" 3 &>/dev/null &) && msg ok "Watch started" || msg warn "Watch gagal"; wait_key; fi
         ;;
      16)
         awk -F'\t' '$11=="1"{print $1}' "$CONFIG_FILE" | while read -r proj; do load_project "$proj"; if [ "$P_TYPE" = "frontend" ] || [ "$P_TYPE" = "fullstack" ]; then supervise_service "$P_NAME" "frontend" "$P_PATH/$P_FE_DIR" "$P_FE_PORT" "$P_FE_CMD"; fi; if [ "$P_TYPE" = "backend" ] || [ "$P_TYPE" = "fullstack" ]; then supervise_service "$P_NAME" "backend" "$P_PATH/$P_BE_DIR" "$P_BE_PORT" "$P_BE_CMD"; fi; done; wait_key
         ;;
      99) uninstall_flow ;;
      0) exit 0 ;;
...
# -----------------------
# Entrypoint: CLI or interactive
# -----------------------
if [ $# -gt 0 ]; then
  case "$1" in
    run) shift; [ $# -ge 1 ] || { msg err "Usage: dapps run <name|#>"; exit 1; }; sel="$1"; load_project "$sel" || { msg err "Project tidak ditemukan: $sel"; exit 1; }; name="$sel"; run_project_flow; exit 0 ;;
    stop) shift; [ $# -ge 1 ] || { msg err "Usage: dapps stop <name|#>"; exit 1; }; sel="$1"; load_project "$sel" || { msg err "Project tidak ditemukan: $sel"; exit 1; }; stop_service "$P_NAME" "frontend"; stop_service "$P_NAME" "backend"; stop_service "$P_NAME" "static"; exit 0 ;;
    add) shift; cli_add_noninteractive "$@"; exit 0 ;;
    status) status_flow; exit 0 ;;
    logs) shift; [ $# -ge 1 ] || { msg err "Usage: dapps logs <name|#>"; exit 1; }; sel="$1"; load_project "$sel" || { msg err "Project tidak ditemukan"; exit 1; }; logs_flow; exit 0 ;;
    export) shift; fname=${1:-dapps-export.json}; if require_cmd jq; then jq -R -s -c 'split("\n")|map(select(length>0))|map(split("\t")|{name:.[0],path:.[1],type:.[2],fe_dir:.[3],be_dir:.[4],fe_port:.[5],be_port:.[6],fe_cmd:.[7],be_cmd:.[8],auto_sync:.[9],supervise:.[10]})' "$CONFIG_FILE" > "$fname"; else awk -F'\t' 'BEGIN{print "["}{if(NR>1)print ","; printf("{\"name\":\"%s\",\"path\":\"%s\",\"type\":\"%s\"}",$1,$2,$3)}END{print "]"}' "$CONFIG_FILE" > "$fname"; fi; msg ok "Exported to $fname"; exit 0 ;;
    import) shift; [ $# -ge 1 ] || { msg err "Usage: dapps import <file>"; exit 1; }; fname="$1"; [ -f "$fname" ] || { msg err "File tidak ditemukan"; exit 1; }; import_config_flow; exit 0 ;;
    watch) shift; [ $# -ge 2 ] || { msg err "Usage: dapps watch <src> <name>"; exit 1; }; watch_and_sync "$1" "$2" & exit 0 ;;
    supervise) shift; [ $# -ge 1 ] || { msg err "Usage: dapps supervise <name|#>"; exit 1; }; sel="$1"; load_project "$sel" || { msg err "Project not found"; exit 1; }; if [ "$P_TYPE" = "frontend" ] || [ "$P_TYPE" = "fullstack" ]; then supervise_service "$P_NAME" "frontend" "$P_PATH/$P_FE_DIR" "$P_FE_PORT" "$P_FE_CMD"; fi; if [ "$P_TYPE" = "backend" ] || [ "$P_TYPE" = "fullstack" ]; then supervise_service "$P_NAME" "backend" "$P_PATH/$P_BE_DIR" "$P_BE_PORT" "$P_BE_CMD"; fi; exit 0 ;;
